...
class Nixy(object):
    ...
    def _verify_local_repo(self):
        ...
        repo_dir = '{}/.nixy/localrepo'.format(home)
        nix_path = 'NIX_PATH=localpkgs={}/default.nix:$NIX_PATH'.format(repo_dir)
        if not 'localpkgs' in os.environ.get('NIX_PATH'):
            self._out('WARNING: set proper NIX_PATH in profile!\nexport {}'.format(nix_path))
            exit(1)
        if os.path.exists('{}/.nixy'.format(home)) is False:
            self._out('no nixy directory exists, creating ...')
            proc = subprocess.run(['mkdir', '-p', '{}/.nixy/'.format(home)])
            proc = subprocess.run(['mkdir', '-p', '{}/.nixy/localrepo'.format(home)])
            proc = subprocess.run(['mkdir', '-p', '{}/.nixy/localrepo/pkgs'.format(home)])
    ...
    def _package_from_scratch(self):
        ...
        p_pkg['symlinks_home'] = True if input('symlinks home? [y/n]: ').lower() == 'y' else False
        p_pkg['bld'] = {}
    ...
    def _process_package(self, pkg_json):
        ...
        pkg_template[meta_index+3] = '    longDescription = "{}";'.format(
            pkg_json['meta']['long_desc'])
    ...
    def _write_package(self, pkg_template, pkg_json):
        ...
        for line in base_nix:
            ...
            if pkg_json['name'] in line and 'callPackage' in line:
                abort_write = True
        pkg_insert = '    {0} = callPackage ./pkgs/{1} {{ }};'.format(
                    pkg_json['name'],
                    pkg_json['name'])
        ...
        for current_dir, subdirs, files in os.walk(path):
            ...
            for fp in files:
                ...
                if root:
                    ...
                    dest_path = '{0}/{1}'.format(base_path, fp)
                    if pkg_json.get('symlinks_home'):
                        dest_path = '{0}/{1}/{2}'.format(home, base_path, fp)
    ...
    def _delete_symlinks(self, path, name, symlinks_home=False):
        root = True
        home = os.path.expanduser('~')
    ...
    def _execute(self):
        if self._args.version:
            self._out('nixy v{}'.format(self._VERSION))
            return
    ...
    def _install(self):
        # todo, determine symlink home by argument local
    ...
    def _uninstall(self):
        ...
        if self._args.local:
            symlinks_home = True
        self._delete_symlinks(self._get_working_dir(target), target, symlinks_home)
        proc = subprocess.run([
            'nix-env',
            '-f',
            "<localpkgs>",
    ...
    def _process_args(self):
        ...
        self._arg_parser.add_argument('-s',
            '--symlinks',
            help='create links',
            action='store_true')
